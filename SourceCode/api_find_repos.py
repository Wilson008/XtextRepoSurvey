...
def search_github_repositories(search_query):
    headers = {
    ...
    while True:
        ...
        url = f'https://api.github.com/search/repositories?q={search_query}+created:{since_date}..{stop_date}+fork:false&page={page}&per_page=100' 
        # url = f'https://api.github.com/search/repositories?q={search_query}&per_page=100&created:{since_date}..{stop_date}&page={page}'
...
def write_to_file(data, page):
    # 将结果写入到对应的文件中
    filename = f'SourceCode/search_results/Search_results_test_{page}.txt'
...
def main():
    search_query = 'Xtext'
    # 调用函数搜索GitHub存储库
    search_github_repositories(search_query)
